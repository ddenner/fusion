    ...
    public class EmailAddressValidator : AbstractValidator<EmailAddress> 
    {
        ...
        public EmailAddressValidator()
        {
            RuleFor(email => email.Email)
                ...
                .NotEmpty()
                .Length(1, 255)
                .WithMessage("Email must not be null and no more than 255 characters.");

            RuleFor(email => email.Email)
                .Must(IsValidEmail)
                .WithMessage("Does not match legal chars for an email.");
        }

        private bool IsValidEmail(string emailAddress)
        {
            return Regex.IsMatch(emailAddress, Expression, RegexOptions.IgnoreCase);
        }
